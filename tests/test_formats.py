...
class TestResolutionsTools(object):
    ...
    def test_listing(self):
        resolution_list = formats.list_matching_resolutions( [1920, 1080] )
        assert( len( resolution_list ) > 2 )

    def test_listing2(self):
        resolution_list = formats.list_matching_resolutions( [1280, 720] )
        assert( len( resolution_list ) > 2 )

    def test_listing_bad_propotions(self):
        resolution_list = formats.list_matching_resolutions( [2, 1] )
